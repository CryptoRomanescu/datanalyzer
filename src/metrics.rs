...
impl WebSocketMetrics {
    ...
    pub fn new() -> Result<Self, prometheus::Error> {
        ...
        let connections_successful = IntCounter::with_opts(Opts::new(
            "websocket_connections_successful",
            "Total number of successful connections",
        ))?;
        registry.register(Box::new(connections_successful.clone()))?;

        let connections_failed = IntCounter::with_opts(Opts::new(
            "websocket_connections_failed",
            "Total number of failed connection attempts",
        ))?;
        registry.register(Box::new(connections_failed.clone()))?;

        let reconnections_total = IntCounter::with_opts(Opts::new(
            "websocket_reconnections_total",
            "Total number of reconnection attempts",
        ))?;
        registry.register(Box::new(reconnections_total.clone()))?;

        let reconnections_successful = IntCounter::with_opts(Opts::new(
            "websocket_reconnections_successful",
            "Total number of successful reconnections",
        ))?;
        registry.register(Box::new(reconnections_successful.clone()))?;

        let reconnections_failed = IntCounter::with_opts(Opts::new(
            "websocket_reconnections_failed",
            "Total number of failed reconnection attempts",
        ))?;
